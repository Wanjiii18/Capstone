    ...
    public function run()
    {
        ...
        foreach ($customers as $customerData) {
            User::firstOrCreate(
                ...
                $customerData // Create with this data if not found
            );
        }
        ...
        foreach ($owners as $ownerData) {
            $owner = User::firstOrCreate(
                ['email' => $ownerData['email']], // Check by email
                $ownerData // Create with this data if not found
            );
            ...
            if ($owner->email !== 'carmen.eatery@email.com') {
                ...
                Karenderia::firstOrCreate(
                    [
                        'owner_id' => $owner->id,
                        'business_name' => $this->getBusinessName($owner->name)
                    ], // Check by owner_id and business_name
                    $karenderiaData // Create with this data if not found
                );
            }
        }
